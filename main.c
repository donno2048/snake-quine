...
#define MAIN(x1,x2,x3,x4,x5,w,h,n,payload) \
int main(){\
    payload;SETUP_SCREEN();int pX=x1,pY=x2,head=x3,tail=x4,dirX=0,dirY=0;char key=0;\
    while(1){\
        SLEEP();GET_KEY(key);\
        switch(key){\
...
MAIN(WIDTH / 2, HEIGHT / 2, 0, 0, %, WIDTH, HEIGHT, NEXT_LOCATION, setup());
        lines[pY][pX] = PLAYER;
